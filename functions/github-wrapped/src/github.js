/**
 * GitHub GraphQL collector — JS port of src/lib/server/wrapped/github.ts.
 * Only aggregates leave this module: no raw events, no code, and private
 * repository names are filtered out before anything is returned.
 */

export class GithubSyncError extends Error {
  /** @param {'rate-limited'|'unauthorized'|'unavailable'} kind */
...
export async function fetchGithubStats(accessToken, period) {
  let res;
  try {
  ...
  if (res.status === 401) {
    throw new GithubSyncError(
      'unauthorized',
      'The GitHub authorization has expired.'
    );
  }
