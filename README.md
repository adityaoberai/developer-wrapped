...
Built end-to-end on **Appwrite**: Auth (GitHub OAuth), TablesDB, Realtime, Functions, and Sites.
...
- **OG images:** rendered at runtime with `satori` + `@resvg/resvg-js` at `/og` (1200×630; `?wrapped=` for published Wrapped pages)
...
- Republishing creates a **new** slug (a new immutable edition); unpublishing deletes the row and the old link stops resolving. Anonymous readers can only ever see `public_shares` rows.
...
The app's own success/failure URLs are derived at runtime from the request origin, preferring trusted `x-forwarded-host`/`x-forwarded-proto` headers when present, so proxied production traffic and localhost both work without config changes. The login route requests only the `read:user` scope.
...
`GET /api/me` · `POST /api/profile` · `GET /api/questions` · `GET /api/answers?user_id=` · `POST /api/answers` · `POST /api/results` · `GET /api/wrapped` (own report) · `POST /api/wrapped` (sync from GitHub) · `POST /api/wrapped/publish` · `DELETE /api/wrapped/publish`
